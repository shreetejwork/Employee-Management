...
const GenerateSalarySlip = () => {
  ...
  const { generateSalarySlip, calculatePreview, fetchSalarySlips } = useSalaryContext();
  ...
  useEffect(() => {
    fetchEmployees();
    fetchSalarySlips();
  }, [fetchEmployees, fetchSalarySlips]);
  ...
  const watchedMonth = watch("salaryMonth");
  const watchedYear = watch("salaryYear");
  ...
  useEffect(() => {
    if (!selectedEmployee || watchedMonth === undefined || !watchedYear) return;

    setPrefilling(true);
    salaryService
      .prefill(selectedEmployee.id, watchedMonth, watchedYear)
      .then((data) => {
        if (!data) return;
      ...
      .catch(() => {
        addToast("Could not prefill attendance data from database", "warning");
      })
      .finally(() => setPrefilling(false));
  }, [selectedEmployee, watchedMonth, watchedYear, setValue, addToast]);
  ...
  const buildPreviewData = (data) => {
    ...
    return {
      ...
      salaryYear: Number(data.salaryYear),
      ...calc,
    };
  };
  ...
  const onGenerate = async (data) => {
    ...
    try {
      ...
      addToast("Salary slip generated and saved successfully!", "success");
      setPreview({ ...slip, salaryMonthName: MONTHS[monthIndex] });
      setShowPreview(true);
    } catch (err) {
      addToast(err.message || "Failed to generate salary slip", "error");
    } finally {
      setGenerating(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
              ...
              {selectedEmployee && (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4 p-4 bg-card rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{selectedEmployee.grade}</p>
                  </div>
                </div>
              )}
            </Card>
            ...
            <Card title="Salary Period">
              ...
              {prefilling && (
                <p className="text-xs text-primary mt-3">Loading attendance data from database...</p>
              )}
            </Card>
            ...
            <Card title="Attendance & Leave">
              ...
              {liveCalc && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 p-3 bg-card rounded-lg text-sm">
                  ...
                  <div>
                    <p className="text-xs text-text-secondary">Cumulative Present</p>
                    <p className="font-medium">{liveCalc.cumulativePresentDays}</p>
                  </div>
                </div>
              )}
            </Card>
